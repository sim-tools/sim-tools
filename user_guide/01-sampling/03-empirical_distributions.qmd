...
In many applications of simulation, theoretical distributions may not be a good fit to the data; for example, if the data are bimodal. A number of empirical distribution options are provided by `sim-tools` to support modelling in these situations
...
We will generate four datasets to illustrate the distribution in use.  One is approx uniform , one right skewed (exponential), one bimodal (a mix of two normal distributions) and finally one with outliers.
...
If the raw data is available then an alternative to Law and Kelton's interpolation method is raw data directly using `RawDiscreteEmpirical`.
