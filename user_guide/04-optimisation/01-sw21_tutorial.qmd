...
A simple model can be created using the `gaussian_sequance_model()` function.  This creates a simulation model where the output of each design follows a $N - (\mu_{i}, 1.0)$ :
...
```{.python}
model = gaussian_sequence_model(1, 10)
```

To create a simulation model with 5 designs where $\mu_{i+1} - \mu_i = 2 $ :

```{.python}
model = gaussian_sequence_model(1, 10, step=2)
```

### Creating a custom model with known designs
Instead of a sequence of normal distributions with unit variance, it is possible to create a custom set of designs with varying variances. Use the `custom_gaussian_model` function for this task.  For example to create a custom set of designs:

```{.python}
means = [5, 8, 1, 2, 1, 7]
variances = [0.1, 1.2, 1.4, 0.3, 0.8]

custom_model = custom_gaussian_model(means, variances)
```
...
model = random_gaussian_model(mean_low, mean_high, var_low, var_high, n_designs)
```
...
**Now have a go yourself**.  This time create a model with random designs.  Run as many replications of each design as you think is necessary to make a decision about which design is best.  Here we define best as the design with the largest mean value.
...
## Procedure **KN**
The first Ranking and Selection (R&S) algorithm we will explore is **Procedure KN** developed by Kim and Nelson. This is an *Indifference Zone* (IZ) approach to R&S.  IZ procedures exploit the idea that the performance of one or more of the sub-optimal designs are in fact so close to the best performing system that a user does not care which one is chosen (the decision maker is said to be *indifferent* to this choice).  To do this a user must specify a quantity $\delta$.  Procedure KN provides a theoretical guarantee to select the best system or a system within $\delta$ of the best with probability of $1 - \alpha$.
...
The first problem we will test KN against is selecting the largest mean from a sequence of 10 normal distributions (means ranging from 1 to 10) with unit variance.  We want a $PCS = 0.95$ and are indifferent to designs that are within an average of 1.0 of the best.  
...
<div class="alert alert-block alert-info">
<b>RUNTIME WARNING!</b> The experiment will take a few seconds to complete. </div>
